...
public class ObjectDetector {
    ...
    private Size getSize(Mat gray, float relativeObjectWidth, float relativeObjectHeight) {
        Size size = gray.size();
        int cameraWidth = gray.cols()/2;
        int cameraHeight = gray.rows()/2;
        int width = Math.round(cameraWidth * relativeObjectWidth)/2;
        int height = Math.round(cameraHeight * relativeObjectHeight)/2;
